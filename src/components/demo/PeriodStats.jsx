...
const PeriodStats = ({ title, userKkmUrl }) => {
    ...
    return (
        <div
            ...
            className={`bg-white dark:text-gray-200 overflow-hidden  dark:bg-secondary-dark-bg rounded-2xl w-[100%] md:w-[45%] p-6 flex flex-col subtle-border`}
        >
                    ...
                    <Stepper ref={stepperRef}>
                        {Object.entries(kkmStats).map(([storeName, storeData]) => {
                            const avgCheck = Math.round(
                                storeData.totalSum / storeData.totalNumberSales,
                            );
                            return (
                                <StepperPanel key={storeName} header={storeName}>
                                    <div className="mt-2">
                                        {financeStatsTemplate.map((stat) => {
                                            const statValue =
                                                stat.valueKey === 'averageCheck'
                                                    ? avgCheck
                                                    : storeData[stat.valueKey];
                                            return (
                                                <div
                                                    key={stat.id}
                                                ...
                                                >
                                                    ...
                                                    <p className={stat.pcColor}>
                                                        {stat.format
                                                            ? FormatAmount(statValue)
                                                            : stat.btrx &&
                                                                stat.title == 'Продаж Bitrix'
                                                              ? FormatAmount(deals.leadsCount)
                                                              : stat.btrx &&
                                                                  stat.title == 'Средний чек Bitrix'
                                                                ? FormatAmount(deals.avgCheck)
                                                                : statValue}{' '}
                                                        {stat.format ? '₸' : ''}
                                                    </p>
                                                </div>
